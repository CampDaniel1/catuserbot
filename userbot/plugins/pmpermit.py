...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"**You were spamming {my_mention}** {my_mention}**'s inbox, henceforth you have been blocked.**"
...
Choose an option from below to specify the reason of your message and wait for me to check it. ⬇️"""
        ...
        USER_BOT_NO_WARN = f"""#AMBOT_Pm_Spam_Protect
        
Hi {mention}, I haven't approved you yet to personal message {my_mention}.

You have {warns}/{totalwarns} warns until you get blocked by the Userbot.

Don't spam {my_mention} inbox. say reason and wait until my response.

ꜱᴜᴘᴘᴏʀᴛ ᴍᴇ ʜᴇʀᴇ 👇🏻🤭
𝓒𝓱𝓪𝓷𝓷𝓮𝓵 -: @tedhub_xd 
𝓐𝓫𝓸𝓾𝓽 𝙏𝙀𝘿 -: @tedteleloif"""
...
async def do_pm_options_action(event, chat):
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = "**If I remember correctly I mentioned in my previous message that this is not the right place for you to spam. \\\x1fThough you ignored that message.So, I simply blocked you. \\\x1fNow you can't do anything unless {my_mention} comes online and unblocks you.**"

    await event.reply(USER_BOT_WARN_ZERO)
    await event.client(functions.contacts.BlockRequest(chat.id))
    the_message = f"#BLOCKED_PM\
                            \n[{get_display_name(chat)}](tg://user?id={chat.id}) is blocked\
                            \n**Reason:** __{my_mention} didn't opt for any provided options and kept on messaging.__"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """__Hey! Have some patience. {my_mention} has not seen your message yet. \
{my_mention} usually responds to people, though idk about some exceptional users.__
__{my_mention} will respond when he comes online, if he wants to.__
**Please do not spam unless you wish to be blocked and reported.**"""
    ...
    the_message = f"#BLOCKED_PM\
                \n[{get_display_name(chat)}](tg://user?id={chat.id}) is blocked\
                \n**Reason:** __{my_mention} opted for enquire option but didn't wait after being told also and kept on messaging so blocked.__"
...
async def do_pm_request_action(event, chat):
    ...
    if str(chat.id) not in PM_WARNS:
        text = """__Hey have some patience. {my_mention} has not seen your message yet. \
{my_mention} usually responds to people, though idk about some exceptional users.__
__{my_mention} will respond when he comes back online, if he wants to.__
**Please do not spam unless you wish to be blocked and reported.**"""
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = "**If I remember correctly I mentioned in my previous message that this is not the right place for you to spam. \\\x1fThough you ignored me and messaged me. So, i simply blocked you. \\\x1fNow you can't do anything unless {my_mention} comes online and unblocks you.**"
...
**Please do not spam unless you wish to be blocked and reported.**"""
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = "**If I remember correctly I mentioned in my previous message this is not the right place for you to spam. \\\x1fThough you ignored that message. So, I simply blocked you. \\\x1fNow you can't do anything unless {my_mention} comes online and unblocks you.**"
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = f"""Ok, Now you are accessing the availabe menu of {my_mention}, {mention}.
...
**Choose one of the following reasons why you are here:**"""
    buttons = [
        ...
        (Button.inline(text="To chat with {my_mention}.", data="to_chat_with_my_master"),),
        (
            Button.inline(
                text="To spam {my_mention}'s inbox.",
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        ...
        return await event.answer(text, cache_time=0, alert=True)
    text = """__Okay. Your request has been registered. Do not spam {my_mention}'s inbox now. \
{my_mention} is busy right now, When {my_mention} comes online he will check your message and ping you. \
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
    text = """__Okay. I have notified {my_mention} about this. When he comes comes online\
 or when {my_mention} is free he will look into this chat and will ping you so we can have a friendly chat.__\
