...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"**ʏᴏᴜ ᴡᴇʀᴇ ꜱᴘᴀᴍᴍɪɴɢ ** {my_mention}**'s ɪɴʙᴏx, ʜᴇɴᴄᴇꜰᴏʀᴛʜ ʏᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ ʙʟᴏᴄᴋᴇᴅ.**"
...
__Hi__ {mention}__, I haven't approved you yet to personal message {my_mention}.

You have {warns}/{totalwarns} warns until you get blocked by the Userbot.

Don't spam {my_mention} inbox. say reason and wait until my response.__
...
async def do_pm_options_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "__Select option from above message and wait. Don't spam {my_mention} inbox, this is your last warning.__"
...
async def do_pm_chat_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """__Heyy! {my_mention} busy right now I already asked you to wait know. After my work finishes. \
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = f"""Ok, Now you are accessing the availabe menu of AMBOT, {mention}.
...
**Choose one of the following reasons why you are here:**"""
    buttons = [
        ...
        (Button.inline(text="To chat with AMBOT.", data="to_chat_with_my_master"),),
        (
            Button.inline(
...
AMBOT is busy right now, When AMBOT comes online he/she will check your message and ping you. \
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
    text = """__Okay. I have notified AMBOT about this. When he comes comes online\
 or when AMBOT is free he will look into this chat and will ping you so we can have a friendly chat.__\
